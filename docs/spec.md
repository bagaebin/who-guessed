...
3) **이미지 생성 1~4기**를 순차 수행하며, 기수별 요구 수량(1·3·7·10장)을 이번 턴의 프롬프트로 생성한다. 생성된 이미지를 AI가 분석해 AppearanceCore를 추출/갱신하고, 각 결과를 빈 캐릭터 타일에 채워 넣는다. 기수 진입 시 필요한 빈 슬롯을 **주변에 추가 생성**해 배치한다(1기: 기존 1칸, 2기 진입 시 총 3칸, 3기 진입 시 7칸까지 확장, 4기 진입 시 10칸까지 확장).
4) 이미지 생성 4기에서는 새롭게 입력된 **네 번째 텍스트**만으로 구성한 동일 프롬프트를 10장 생성에 적용하고, 생성된 이미지 분석 결과로 모든 AppearanceCore를 덮어쓴다(기존 분기 추론 체인은 보관하지 않으며 턴별 프롬프트에만 의존). 생성 직후 AppearanceCore 값이 고정되며 이후 제거 단계에서 활용한다.
...
- **이미지 생성 2기**: 3장 생성. 같은 턴의 질문+답변 프롬프트를 3장 배치 생성해 분석·AppearanceCore를 추출하고, 1기 슬롯 주변에 추가된 빈 슬롯에 채워 넣음(총 3칸).
- **이미지 생성 3기**: 7장 생성. 해당 턴의 질문·답변으로 작성한 프롬프트를 7장 배치 생성해 분석한 AppearanceCore 7개로 슬롯을 7칸까지 확장해 채움(분기 체인 미사용, 턴 프롬프트 단발).
- **이미지 생성 4기**: 10장 생성. **네 번째 질문·답변만**으로 구성한 프롬프트를 10장 생성·분석해 AppearanceCore 10개를 최신화하고, 슬롯을 10칸까지 확장해 채움. AppearanceCore는 생성 직후 고정되며 제거 단계에서 그대로 사용한다.
