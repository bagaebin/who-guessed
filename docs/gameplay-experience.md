...
2. 플레이어가 텍스트를 입력하면 `submitPlayerText`가 호출되고, 텍스트 기반 AppearanceCore를 기수별로 추론한다.
3. 이미지 생성 1~4기를 순차 진행하며 기수별 요구 수량(1·3·8·12장)을 충족할 만큼 AppearanceCore를 출력하고, 각 추론을 이미지화해 빈 타일에 채워 넣는다.
4. 기수 진입 시 필요한 빈 슬롯을 주변에 추가 생성해 확장한다: 1기(1칸) → 2기 진입 시 3칸 추가(총 4칸) → 3기 진입 시 8칸까지 확장 → 4기 진입 시 12칸까지 확장.
5. 2~4기에서는 누적된 플레이어 응답 맥락을 고려한다. 2기는 새롭게 3개 추론을 생성하며, 3기는 1기+2기에서 확보한 4개 추론 체인을 각각 2개씩 분기(총 8개), 4기는 **네 번째 텍스트 입력으로 기존 12개 추론 체인의 AppearanceCore 값을 모두 갱신**한 뒤 이미지화한다.
6. 기수별 이미지 생성이 끝날 때마다 다음 질문을 제공하고, 이미지 생성 4기 종료 후 기존 제거 단계(early/mid/late)로 전환한다.
...
## 4. 단계(phase) 규칙
- **이미지 생성 1기**: 1장 생성 (초기 AppearanceCore 1개 → 이미지 1장, 초기 단일 슬롯 사용).
- **이미지 생성 2기**: 3장 생성 (독립 추론 3개 → 이미지 3장, 1기 슬롯 주변에 추가된 3칸에 배치).
- **이미지 생성 3기**: 8장 생성 (1·2기 추론 4개 체인 각각 2개로 분기 → 8장, 슬롯을 8칸까지 확장해 채움).
- **이미지 생성 4기**: 12장 생성 (네 번째 텍스트로 기존 12개 추론 체인을 모두 갱신 → 12장, 슬롯을 12칸까지 확장해 채움).
- **early**: 3–4장 제거
- **mid**: 2–3장 제거
- **late**: 1–2장 제거
