...
5. 2~4기에서는 누적된 플레이어 응답 맥락을 반영한 프롬프트를 사용한다. 4기에서는 **네 번째 텍스트를 반영한 동일 프롬프트로 12장 생성** 후 이미지 분석 결과로 AppearanceCore를 덮어쓴다(분기 추론 체인은 보존하지 않음).
...
- **이미지 생성 3기**: 8장 생성 (누적 응답을 반영한 프롬프트 8장 → 이미지 분석으로 AppearanceCore 8개, 슬롯을 8칸까지 확장해 채움).
- **이미지 생성 4기**: 12장 생성 (네 번째 텍스트를 포함한 프롬프트 12장 → 이미지 분석으로 AppearanceCore 12개 최신화, 슬롯을 12칸까지 확장해 채움).
- **early**: 3–4장 제거
- **mid**: 2–3장 제거
- **late**: 1–2장 제거
