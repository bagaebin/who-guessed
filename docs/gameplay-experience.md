...
3. 이미지 생성 1~4기를 순차 진행하며 기수별 요구 수량(1·3·8·12장)을 한 번의 프롬프트로 생성한다. 생성된 각 이미지를 AI가 분석해 AppearanceCore를 추출/갱신하고, 빈 타일에 채워 넣는다.
4. 기수 진입 시 필요한 빈 슬롯을 주변에 추가 생성해 확장한다: 1기(1칸) → 2기 진입 시 3칸 추가(총 4칸) → 3기 진입 시 8칸까지 확장 → 4기 진입 시 12칸까지 확장.
5. 각 기수는 **해당 턴의 질문·답변으로 만든 프롬프트**만 사용해 요구 장수를 생성하며, 생성 직후 이미지를 분석해 도출된 AppearanceCore를 타일에 기록·고정한다(분기 추론 체인 미사용).
6. 이미지 생성 4기에서도 네 번째 질문·답변으로만 구성한 프롬프트로 12장을 생성·분석해 AppearanceCore를 최신화한다. 기수별 이미지 생성이 끝날 때마다 다음 질문을 제공하고, 이미지 생성 4기 종료 후 기존 제거 단계(early/mid/late)로 전환한다.
...
- **이미지 생성 2기**: 3장 생성 (질문+답변 프롬프트 배치 3장 → 이미지 분석으로 AppearanceCore 3개, 1기 슬롯 주변에 추가된 3칸에 배치).
- **이미지 생성 3기**: 8장 생성 (해당 턴의 질문·답변 프롬프트 8장 → 이미지 분석으로 AppearanceCore 8개, 슬롯을 8칸까지 확장해 채움).
- **이미지 생성 4기**: 12장 생성 (네 번째 질문·답변 프롬프트 12장 → 이미지 분석으로 AppearanceCore 12개 최신화, 슬롯을 12칸까지 확장해 채움; 생성 시점에 AppearanceCore를 고정하고 이후 제거 단계에서 사용).
- **early**: 3–4장 제거
- **mid**: 2–3장 제거
- **late**: 1–2장 제거
