...
## 3. 게임 흐름
### 이미지 생성 전반부
1. 게임 시작 시 플레이어 카메라 타일을 제외한 중앙에는 **빈 슬롯 1개만** 배치되어 있으며, 24칸 전체를 한 번에 노출하지 않는다.
...
3. 이미지 생성 1~4기를 순차 진행하며 기수별 요구 수량(1·3·7·10장)을 한 번의 프롬프트로 생성한다. 생성된 각 이미지를 AI가 분석해 AppearanceCore를 추출/갱신하고, 빈 타일에 채워 넣는다.
4. 기수 진입 시 필요한 빈 슬롯을 주변에 추가 생성해 확장한다: 1기(1칸) → 2기 진입 시 총 3칸 → 3기 진입 시 7칸까지 확장 → 4기 진입 시 10칸까지 확장.
5. 각 기수는 **해당 턴의 질문·답변으로 만든 프롬프트**만 사용해 요구 장수를 생성하며, 생성 직후 이미지를 분석해 도출된 AppearanceCore를 타일에 기록·고정한다(분기 추론 체인 미사용).
6. 이미지 생성 4기에서도 네 번째 질문·답변으로만 구성한 프롬프트로 10장을 생성·분석해 AppearanceCore를 최신화한다. 기수별 이미지 생성이 끝날 때마다 다음 질문을 제공하고, 이미지 생성 4기 종료 후 기존 제거 단계(early/mid/late)로 전환한다.

### 기존 제거 루프
6. 남은 타일과 단계(early/mid/late)를 함께 LLM 엔드포인트에 POST한다.
7. 응답(`profile`, `eliminatedIds`, `reasoning`)을 Zod로 검증 후, 제거 ID를 단계별 허용 범위에 맞춰 보정(최소/최대 제거 수, 마지막 1장 보호).
8. 응답마다 `playerProfile`을 최신 추론 결과로 갱신하고, 제거 ID에 따라 타일 `isEliminated` 토글 + flip 애니메이션.
9. 남은 타일 수 기반으로 phase 재계산, round 증가. 1장 이하이면 종료 메시지 노출.
10. 실패 시 mock 응답 + 상태 메시지로 사용자에게 알림.

## 4. 단계(phase) 규칙
- **이미지 생성 1기**: 1장 생성 (질문+답변 프롬프트 1장 → 이미지 분석으로 AppearanceCore 도출, 초기 단일 슬롯 사용).
- **이미지 생성 2기**: 3장 생성 (질문+답변 프롬프트 배치 3장 → 이미지 분석으로 AppearanceCore 3개, 1기 슬롯 주변에 추가된 슬롯에 배치하여 총 3칸 채움).
- **이미지 생성 3기**: 7장 생성 (해당 턴의 질문·답변 프롬프트 7장 → 이미지 분석으로 AppearanceCore 7개, 슬롯을 7칸까지 확장해 채움).
- **이미지 생성 4기**: 10장 생성 (네 번째 질문·답변 프롬프트 10장 → 이미지 분석으로 AppearanceCore 10개 최신화, 슬롯을 10칸까지 확장해 채움; 생성 시점에 AppearanceCore를 고정하고 이후 제거 단계에서 사용).
- **early**: 3–4장 제거
- **mid**: 2–3장 제거
- **late**: 1–2장 제거
