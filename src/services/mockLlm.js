...
export function mockInferPlayerAppearance(playerText, remainingTiles) {
    const phase = getPhaseByRemaining(remainingTiles.length);
    const { min, max } = getEliminationRange(phase);
    const eliminationTarget = Math.min(Math.max(min, Math.ceil(remainingTiles.length * 0.25)), max, remainingTiles.length - 1);
    ...
    const reasoning = Object.fromEntries(eliminatedIds.map((id) => [id, `${playerText.slice(0, 30)}... 와 다른 핵심 특징`]));
    return {
